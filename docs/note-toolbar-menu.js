/*
 * MD Annotation — Note Toolbar "Apply format" menu
 * ------------------------------------------------
 * Paste this into a Note Toolbar item of type **JavaScript**
 * (Note Toolbar → enable Other → Scripting → add a JavaScript item).
 *
 * Clicking the toolbar item opens a menu listing every annotation format
 * defined in MD Annotation. Picking one applies that format to the current
 ...
 * formats in the plugin settings — nothing here needs editing.
 *
 * How it works: MD Annotation registers one command per format
...
if (items.length === 0) {
  new Notice('MD Annotation: no formats defined');
} else {
